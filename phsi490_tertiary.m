...
i = 1;
while true
    McIlwain_forward(i) = only_McIlwain(3113+i); %#ok<SAGROW>
    del_McIlwain = only_McIlwain(3113+i)-only_McIlwain(3113+i-1);
    i = i + 1;
    if only_McIlwain(3113+i) < 1.2 || del_McIlwain <= 0
        break
    end
end
...
i = 1;
while true
   McIlwain_backward(i) = only_McIlwain(3243+i); %#ok<SAGROW>
   del_McIlwain = only_McIlwain(3243+i+1)-only_McIlwain(3243+i);
   i = i + 1;
   if only_McIlwain(3243+i) < 1.2 || del_McIlwain >= 0
       break
   end
...
L = 1/((cosd(70))^2);
pf_for = Omni_directional_P6(24901:8:25461);
pf_back = Omni_directional_P6(25941:8:26817);
...
plot(for_L*ones(1,501),0:1:500,'b-.')
text(double(for_L),400,lforlabel)
plot(back_L*ones(1,501),0:1:500,'r-.')
text(double(back_L),400,lbacklabel)
